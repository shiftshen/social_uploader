...
6.  **启动后端项目**:
    ...
    ```
    后端项目将在 `http://localhost:5406` 启动。

7.  **启动前端项目**:
    ```bash
    cd sau_frontend
    npm install
    npx vite --host 0.0.0.0 --port 5106 --strictPort
    ```
    前端项目将在 `http://localhost:5106` 启动，在浏览器中打开此链接即可访问。

### Windows 一键启动

已在项目根目录提供 `start-all.bat` 脚本：

- 双击运行即可同时启动后端（端口 `5406`）和前端（端口 `5106`）
- 首次运行会自动安装前端依赖（如未安装）

文件路径：
- 启动脚本：`new\start-all.bat`
- 后端入口：`new\sau_backend.py`
- 前端目录：`new\sau_frontend`

常见问题：
- 若浏览器无法访问根路径 `/`，请直接访问前端 `http://localhost:5106/`；后端根路径可能返回 500，不影响接口可用性
- 如果端口被占用，请关闭占用进程或调整端口（前端通过 `--port` 指定）

### Windows 一键关闭

已在项目根目录提供 `stop-all.bat` 脚本，用于一键关闭前后端：

- 通过端口结束相关进程：`5406`（后端）、`5106`（前端）
- 双击运行即可关闭

文件路径：
- 关闭脚本：`new\stop-all.bat`

## 最新开启方式（推荐）

1. 双击 `new\stop-all.bat`
2. 双击 `new\start-all.bat`
3. 打开前端：`http://localhost:5106/#/publish-center` 或 `http://localhost:5106/#/account-management`
4. 后端健康检查：访问 `http://localhost:5406/getValidAccounts` 与 `http://localhost:5406/getFiles`

说明：
- 脚本固定端口：后端 `5406`、前端 `5106`
- 启动前自动清理端口占用，前端使用 `npx vite --host 0.0.0.0 --port 5106 --strictPort`
